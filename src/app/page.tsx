...
import { Heart, Wind, Sparkles, Lightbulb, MessageCircle, Calendar, Flame, Send, X, BookOpen, BarChart2 } from 'lucide-react';
...
export default function SahayApp() {
  ...
  const [activeTab, setActiveTab] = useState<'checkin'|'trends'>('checkin');
    ...
    <div className="app-shell">
      ...
      <header className="app-topbar">
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div className="badge badge-teal"><Calendar size={12}/>{DAYS_LEFT} days to {exam}</div>
      ...
      <aside className="app-sidebar">
        <div>
          <p className="section-label">Exam</p>
          <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
            {EXAMS.map(e=>(
              <button key={e} className={`exam-chip ${exam===e?'active':''}`} onClick={()=>setExam(e)}>{e}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            ...
            <button className={`nav-item ${activeTab==='trends'?'active':''}`} onClick={()=>setActiveTab('trends')}>
              <BarChart2 size={16}/> Mood Trends
            </button>
            <button className="nav-item" onClick={()=>setIsChatOpen(true)}>
              <MessageCircle size={16}/> Talk it Out
            </button>
          </div>
        </div>

        {/* Mini chart in sidebar */}
        <div className="card" style={{padding:'20px 16px',marginTop:'auto'}}>
          <p style={{fontSize:12,fontWeight:700,color:'var(--text-muted)',marginBottom:12,textTransform:'uppercase',letterSpacing:'0.07em'}}>This week</p>
          <div style={{height:80}}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={moodHistory} margin={{top:4,right:4,left:-30,bottom:0}}>
                <defs>
                  <linearGradient id="sg" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="10%" stopColor="var(--accent)" stopOpacity={0.25}/>
                ...
                <Area type="monotone" dataKey="score" stroke="var(--accent)" strokeWidth={2} fill="url(#sg)" animationDuration={1200}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="stagger">
          ...
          {activeTab === 'trends' && (
            <div className="card" style={{padding:'32px 36px'}}>
              ...
              <p style={{fontSize:14,color:'var(--text-secondary)',marginBottom:28}}>Mood logged over the last 7 days (1 = awful, 5 = great)</p>
